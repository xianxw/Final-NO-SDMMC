...
vf2-write-perf-test:
	$(MAKE) ARCH=riscv64 APP_FEATURES="vf2 sdmmc-write-perf-test" \
		MYPLAT=axplat-riscv64-visionfive2 BUS=mmio \
		TARGET_DIR=$(PWD)/target/vf2-write-perf-test \
		OUT_DIR=$(PWD)/target/vf2-write-perf-test-image build
	cp $(PWD)/target/vf2-write-perf-test-image/Final-NO-SDMMC_visionfive2.elf \
		$(PWD)/Final-NO-SDMMC_visionfive2_write_perf_test.elf
	cp $(PWD)/target/vf2-write-perf-test-image/Final-NO-SDMMC_visionfive2.bin \
		$(PWD)/Final-NO-SDMMC_visionfive2_write_perf_test.bin

.PHONY: build run justrun debug disasm clean vf2 vf2-write-perf-test
