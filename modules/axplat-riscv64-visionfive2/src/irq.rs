...
impl IrqIf for IrqIfImpl {
    ...
    fn set_enable(irq: usize, enabled: bool) {
        with_cause!(
            ...
            @EX_IRQ => {
                ...
                if enabled {
                    info!("PLIC enable IRQ {} for context {}", irq, this_context());
    ...
    fn handle(irq: usize) -> Option<usize> {
        with_cause!(
            ...
            @S_EXT => {
                debug!("IRQ: external interrupt entered, reading PLIC claim");
                let mut plic = PLIC.lock();
                let Some(irq) = plic.claim(this_context()) else {
                    debug!("Spurious external IRQ");
                    return None;
                };
                debug!("IRQ: external {} claimed, dispatching handler", irq.get());
                IRQ_HANDLER_TABLE.handle(irq.get() as usize);
                debug!("IRQ: external {} handler returned, completing PLIC", irq.get());
