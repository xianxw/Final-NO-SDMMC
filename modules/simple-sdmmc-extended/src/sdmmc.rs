...
// BEGIN SDMMC TEST ONLY - all diagnostic test logic lives in one removable file.
#[cfg(any(
...
impl SdMmc {
    ...
    pub unsafe fn new(base: usize, register_irq: impl FnOnce() -> bool) -> Self {
        ...
        {
            this.run_async_read_and_cancel_test()
        }
        #[cfg(feature = "sdmmc-error-irq-test")]
        {
            this.run_error_irq_test()
        }
    ...
    fn log_register_snapshot(&self) {
        ...
        warn!(
            "SDMMC_REGISTER_SNAPSHOT stage=pre_init HCON=0x{hcon:08x} \
             FIFOTH=0x{fifoth:08x} BMOD=0x{bmod:08x} TMOUT=0x{tmout:08x} \
             CLKSRC=0x{clksrc:08x} UHS=0x{uhs:08x} CTRL=0x{ctrl:08x}"
        );
    }
    ...
    fn read_dma_chunk(&mut self, block: u32, buf: &mut [u8]) -> SdMmcResult {
        ...
        let dma_buf_info = self
            .dma_buffer
            .as_ref()
            .ok_or(SdMmcError::DmaUnavailable)?;
    ...
    pub fn try_enable_idmac(
        ...
        let Ok(layout) = Layout::from_size_align(buf_size, hardware_data_width.align_value()) else {
        ...
        if !bmod_after.de() || bmod_after.dsl() != 0 || !bmod_after.fb() {
            warn!(
                "Failed to set BMOD register for IDMAC; block transfers are unavailable; actual: de={}, \
                 dsl={}, fb={}, pbl={}",
                bmod_after.de(),
                bmod_after.dsl(),
                bmod_after.fb(),
    ...
    fn prepare_idmac_transfer(
        ...
        for index in 0..descriptor_count {
            ...
            let next_descriptor_addr = if last {
                ...
                desc_phy_addr
                    + ((index + 1) * core::mem::size_of::<IdmacDescriptor>()) as u32
            };
            ...
            || owned_descriptors != descriptor_count
        {
            ...
            return Err(self.abort_idmac_transfer(
                context,
    ...
    async fn wait_transfer_async(
        ...
        if context.cmd.response_expect() {
            ...
            if command_timed_out {
                ...
                {
                    let (rintsts, idsts) = self.idmac_completion_status(context.generation);
                    diagnostic_tests::record_async_resume(rintsts, idsts);
                    #[cfg(feature = "sdmmc-error-irq-test")]
                    diagnostic_tests::record_async_resume_registers(
                        self.regs.mintsts().read().into_bits(),
                        self.regs.intmask().read().into_bits(),
                        self.regs.idinten().read().into_bits(),
    ...
    async fn send_cmd_idmac_async(
        ...
        diagnostic_tests::reset_async_observation();
        // END SDMMC TEST ONLY
        ...
        if let Err(error) = transfer.wait_async().await {
            ...
            log::error!(
                "SDMMC DMA error outside IRQ: async=true cmd={} error={:?} \
    ...
    fn send_cmd_idmac(
        ...
        if let Err(error) = transfer.wait_sync() {
            let (rintsts, idsts) = transfer.status();
            log::error!(
                "SDMMC DMA error outside IRQ: async=false cmd={} error={:?} \
                 RINTSTS=0x{:08x} IDSTS=0x{:08x}",
                transfer.context().cmd.cmd_index(),
                error,
                rintsts.into_bits(),
    ...
    pub fn dma_irq_handler() {
        ...
        let mut irq_entry_ns = 0;
        #[cfg(feature = "sdmmc-error-irq-test")]
        let error_irq_trace_ordinal = diagnostic_tests::begin_error_irq_handler_entry();
        // END SDMMC TEST ONLY
        if regs_base != 0 {
            // BEGIN SDMMC TEST ONLY
            #[cfg(any(
            ...
            {
                irq_entry_ns = axhal::time::monotonic_time_nanos();
            }
            // END SDMMC TEST ONLY
            ...
            // BEGIN SDMMC TEST ONLY
            #[cfg(feature = "sdmmc-error-irq-test")]
            let mintsts_bits = regs.mintsts().read().into_bits();
            // END SDMMC TEST ONLY
            ...
            // BEGIN SDMMC TEST ONLY
            #[cfg(feature = "sdmmc-error-irq-test")]
            diagnostic_tests::record_error_irq_initial_status(
                error_irq_trace_ordinal,
        ...
        if should_notify {
            ...
            diagnostic_tests::record_irq_wake(
                irq_entry_ns,
            ...
            // BEGIN SDMMC TEST ONLY
            #[cfg(feature = "sdmmc-error-irq-test")]
            diagnostic_tests::record_error_irq_notify(
                error_irq_trace_ordinal,
                _waiter_woken,
            );
            // END SDMMC TEST ONLY
