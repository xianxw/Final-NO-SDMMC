...
const INITIAL_SLAB_HEAP_PAGES: usize = 8;
...
impl DmaAllocator {
    ...
    unsafe fn ensure_initialized(&mut self) -> AllocResult<()> {
        if self.initialized {
            return Ok(());
        }

        let num_pages = INITIAL_SLAB_HEAP_PAGES;
        let vaddr_raw =
        ...
        self.alloc.init(vaddr_raw, num_pages * PAGE_SIZE_4K);
    ...
    fn alloc_coherent_bytes(&mut self, layout: Layout) -> AllocResult<DMAInfo> {
        if unsafe { self.ensure_initialized() }.is_err() {
            debug!("slab DMA heap init failed, falling back to page allocation");
            let mut dma = self.alloc_coherent_pages(layout)?;
            dma.page_allocated = true;
            return Ok(dma);
        }
        ...
        #[cfg(not(feature = "hv"))]
        {
            ...
            // If the byte allocator cannot serve this allocation, fall back to a
            // page-based coherent allocation.
            let mut dma = self.alloc_coherent_pages(layout)?;
            dma.page_allocated = true;
            Ok(dma)
        }
    ...
    pub unsafe fn dealloc_coherent(&mut self, dma: DMAInfo, layout: Layout) {
        if dma.page_allocated || layout.size() >= PAGE_SIZE_4K {
            let num_pages = layout_pages(&layout);
